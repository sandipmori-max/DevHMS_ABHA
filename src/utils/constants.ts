...
let dynamicAppColor: string | null = null; // 👈 NEW

export const setERPTheme = (theme: 'light' | 'dark') => {
  currentTheme = theme;
};

// 👇 NEW function to update color
export const setERPAppColor = (color: string) => {
  dynamicAppColor = color;
};
...
export const ERP_COLOR_CODE = new Proxy(lightColors, {
  get(target, prop: string) {
    ...
    // 👇 override only for APP COLOR
    if (prop === 'ERP_APP_COLOR' && dynamicAppColor) {
      return dynamicAppColor;
    }

    ...
    switch (item?.type) {
      ...
      case "action":
        if (item?.action === "Logout") {
          ...
          setAlertConfig({
            title: t("settings.logout"),
            message: `${firstLetterUpperCase(user?.name || "")}, ${t(
    ...
    >
      <View style={styles.settingHeader}>
        ...
        {item.type === "toggle" ? (
          <Switch
            value={
              item.title === t("settings.darkMode")
                ? theme === "dark"
                : item.value
            }
            onValueChange={() => {
            ...
            trackColor={{
              false:
                Platform.OS === "ios" ? "#e5e7eb" : ERP_COLOR_CODE.ERP_e0e0e0,
              true: "#4CAF50",
            }}
            thumbColor={
              Platform.OS === "android"
          ...
          <>
            {item?.title !== t("settings.aboutApp") && (
